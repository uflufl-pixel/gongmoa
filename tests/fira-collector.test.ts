...
test('FIRA imports the standing vessel-owner opportunity conservatively',()=>{const r=collectFiraBundle(bundle());assert.equal(r.items.length,1);assert.equal(r.items[0].externalId,'2026-06');assert.equal(r.items[0].status,'unknown');assert.equal(r.items[0].closesAt,null);assert.equal(r.items[0].supportBudget,null);});
