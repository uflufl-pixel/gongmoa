...
type Notice = { id:string; org:string; group:string; title:string; due:string; dday:number; tag:string; audience:string; source:string; url:string; reviewReason:string|null };
...
const fallback: Notice[] = [
  {id:'govtech-2026',org:'과학기술정보통신부',group:'중앙부처',title:'2026년 GovTech 창업 경진대회 모집 공고',due:'2026.09.21',dday:20,tag:'창업',audience:'기업·예비창업자',source:'기업마당',url:'https://www.bizinfo.go.kr/sii/siia/selectSIIA200View.do',reviewReason:null},
  {id:'export-logistics-2026',org:'중소벤처기업부',group:'중앙부처',title:'2026년 2차 온라인수출 중소기업 물류 지원 사업',due:'2026.09.16',dday:15,tag:'수출',audience:'중소기업',source:'기업마당',url:'https://www.bizinfo.go.kr/sii/siia/selectSIIA200View.do',reviewReason:null},
  {id:'andong-healingroad-2026',org:'경상북도',group:'지방자치단체',title:'2026 안동 낙동강 힐링로드 보조사업 지원신청 공모',due:'2026.09.14',dday:13,tag:'지역',audience:'기관·단체',source:'보조금통합포털',url:'https://www.bojo.go.kr/retrieveSearchPubBiz.do',reviewReason:null},
  {id:'open-innovation-2026',org:'중소벤처기업부',group:'중앙부처',title:'민관협력 오픈이노베이션 지원 창업기업 모집',due:'2026.09.09',dday:8,tag:'기술',audience:'창업기업',source:'기업마당',url:'https://www.bizinfo.go.kr/sii/siia/selectSIIA200View.do',reviewReason:null},
  {id:'school-complex-2026',org:'교육부',group:'중앙부처',title:'2026년 제2차 학교복합시설 공모사업',due:'공고문 확인',dday:99,tag:'교육',audience:'지방자치단체',source:'교육부',url:'https://www.moe.go.kr/boardCnts/listRenew.do?boardID=72761&m=020502&s=moe',reviewReason:'마감일 확인'},
];
...
export default function Home() {
  ...
  const filtered=useMemo(()=>notices.filter(n=>reviews[n.id]!=='excluded'&&(group==='전체'||n.group===group)&&(!showSaved||saved.includes(n.id))&&(!reviewOnly||(n.reviewReason&&!reviews[n.id]))&&(`${n.org} ${n.title} ${n.tag} ${n.audience}`).toLowerCase().includes(query.toLowerCase())),[notices,query,group,saved,showSaved,reviewOnly,reviews]);
  ...
  const runSync=async()=>{setSyncing(true);try{const response=await fetch('/api/sync',{method:'POST'});if(!response.ok)throw new Error();const data=await response.json() as {results:SourceCheck[];collection:CollectionSummary};setChecks(data.results);setCollection(data.collection);const refreshed=await fetch('/api/notices').then(r=>r.json()) as {items:Array<{id:string;institution:string;group:string;title:string;deadlineLabel:string;closesAt:string|null;category:string;audience:string;sourceName:string;sourceUrl:string}>};setNotices(refreshed.items.map(n=>({id:n.id,org:n.institution,group:n.group,title:n.title,due:n.deadlineLabel,dday:daysUntil(n.closesAt),tag:n.category,audience:n.audience,source:n.sourceName,url:n.sourceUrl,reviewReason:getReviewReason(n.title,n.closesAt)})));}finally{setSyncing(false)}};
  return <main>
    ...
    <section className="overview"><div><b>{notices.filter(n=>reviews[n.id]!=='excluded').length}</b><span>진행 중 공모</span></div><div><b>{pendingCount}</b><span>검토 필요</span></div><div><b>10</b><span>연결된 공식 출처</span></div><div><b className={mode==='live-db'?'live':'pending'}>{mode==='live-db'?'LIVE':mode==='fallback'?'표본':'연결 중'}</b><span>데이터 저장소</span></div></section>
    <section className="content" id="notices"><div className="sectionhead"><div><span>OPEN CALLS</span><h2>{reviewOnly?'검토 대기 공모':showSaved?'저장한 공모':'공모사업 둘러보기'}</h2></div><p>{filtered.length}건 표시 · {mode==='live-db'?'영구 저장소 동기화':'검증 표본'}</p></div><div className="filters" role="group" aria-label="기관 유형 필터">{groups.map(g=><button className={group===g?'selected':''} onClick={()=>setGroup(g)} key={g}>{g}</button>)}<button className={reviewOnly?'selected reviewFilter':'reviewFilter'} onClick={()=>setReviewOnly(v=>!v)}>검토 필요 {pendingCount}</button></div>
      {filtered.length?<div className="noticegrid">{filtered.map((n,i)=><article key={n.id} className="notice"><div className="meta"><span className={`tag t${i%4}`}>{n.tag}</span><button onClick={()=>toggle(n.id)} aria-label="관심 공모 저장">{saved.includes(n.id)?'★':'☆'}</button></div>{n.reviewReason&&!reviews[n.id]&&<><div className="reviewBadge">검토 · {n.reviewReason}</div><div className="reviewActions"><button onClick={()=>review(n.id,'approved')}>공모 승인</button><button onClick={()=>review(n.id,'excluded')}>목록 제외</button></div></>}<p>{n.group} · {n.org}</p><h3><a href={n.url} target="_blank" rel="noreferrer">{n.title}</a></h3><div className="audience">지원대상 · {n.audience}</div><footer><strong>{n.due}</strong><span>{n.dday<90?`D-${n.dday}`:'확인 필요'}</span></footer><a className="source" href={n.url} target="_blank" rel="noreferrer">{n.source} 원문 ↗</a></article>)}</div>:<div className="empty">조건에 맞는 공모가 없습니다. 검색어나 기관 유형을 바꿔보세요.</div>}
    </section>
    ...
    <section className="syncCenter"><div className="syncHead"><div><span>SOURCE MONITOR</span><h2>공식 출처 수집 상태</h2><p>원문 접근 여부와 콘텐츠 해시를 저장하고, 공개 목록의 실제 공고를 중복 없이 반영합니다.</p></div><button onClick={runSync} disabled={syncing}>{syncing?'수집 중…':'지금 공고 수집'}</button></div>{collection&&<div className="collectionSummary"><span>발견 <b>{collection.discovered}</b></span><span>신규 <b>{collection.inserted}</b></span><span>변경 <b>{collection.updated}</b></span><span>동일 <b>{collection.unchanged}</b></span><span>마감분리 <b>{collection.closed}</b></span></div>}<div className="syncGrid">{checks.length?checks.slice(0,10).map(check=><article key={check.id}><div><i className={check.outcome==='success'?'ok':'warn'}></i><strong>{check.pageTitle||check.sourceId}</strong></div><p>{check.outcome==='success'?`정상 · 키워드 ${check.keywordHits??0}회 · ${Math.round((check.contentBytes||0)/1024).toLocaleString()}KB`:check.message||'확인 필요'}</p><time>{new Date(check.finishedAt).toLocaleString('ko-KR')}</time></article>):<div className="syncEmpty">아직 실행 이력이 없습니다. 수집을 실행하면 상태와 변경 기준 해시가 저장됩니다.</div>}</div><small>기업마당·5개 중앙부처·서울·부산·인천 공개 목록과 기획예산처 OpenAPI를 함께 수집합니다.</small></section>
    <section className="sources"><h2>연결된 공식 출처</h2><div><a href="https://www.bojo.go.kr/retrieveSearchPubBiz.do" target="_blank">보조금통합포털 <small>기획예산처 API</small></a><a href="https://www.bizinfo.go.kr/sii/siia/selectSIIA200View.do" target="_blank">기업마당 <small>공개 게시판</small></a><a href="https://www.moe.go.kr/boardCnts/listRenew.do?boardID=72761&m=020502&s=moe" target="_blank">교육부 <small>사업공고</small></a><a href="https://www.mcst.go.kr/site/s_notice/notice/noticeList.jsp" target="_blank">문화체육관광부 <small>공지</small></a><a href="https://www.mois.go.kr/frt/bbs/type013/commonSelectBoardList.do?bbsId=BBSMSTR_000000000006" target="_blank">행정안전부 <small>알립니다</small></a><a href="https://me.go.kr/home/web/board/list.do?boardMasterId=39&menuId=10524" target="_blank">기후에너지환경부 <small>공지·공고</small></a><a href="https://www.seoul.go.kr/news/news_notice.do?selmenu=M00000107" target="_blank">서울특별시 <small>고시·공고</small></a><a href="https://www.busan.go.kr/nbgosi" target="_blank">부산광역시 <small>고시공고</small></a><a href="https://www.incheon.go.kr/IC010307/list?curPage=1" target="_blank">인천광역시 <small>고시공고</small></a><a href="https://www.gov.kr/portal/orgSite" target="_blank">정부24 <small>기관 기준정보</small></a></div><p>기관별 공개 게시판은 3시간 주기로 구조 변경과 신규 공고를 함께 확인합니다.</p></section>
