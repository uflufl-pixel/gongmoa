...
export default function Home() {
  ...
  return <main>
    ...
    <section className="overview"><div><b>{notices.filter(n=>reviews[n.id]!=='excluded').length}</b><span>진행 중 공모</span></div><div><b>{pendingCount}</b><span>검토 필요</span></div><div><b>4</b><span>연결된 공식 출처</span></div><div><b className={mode==='live-db'?'live':'pending'}>{mode==='live-db'?'LIVE':mode==='fallback'?'표본':'연결 중'}</b><span>데이터 저장소</span></div></section>
