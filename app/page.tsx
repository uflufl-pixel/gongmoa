...
export default function Home() {
  ...
  const pendingCount=notices.filter(n=>verification(n).status!=='verified'&&reviews[n.id]!=='excluded').length;
  ...
  return <main>
    ...
    <section className="overview"><div><b>{verifiedCount}</b><span>본문 확인 공모사업</span></div><div><b>{pendingCount}</b><span>미확인 수집 후보</span></div><div><b>{sourceCount??'—'}</b><span>등록된 공고 수집 출처</span></div><div><b className={mode==='live-db'?'live':'pending'}>{mode==='live-db'?'LIVE':mode==='fallback'?'연결 실패':'연결 중'}</b><span>데이터 저장소</span></div></section>
    <section className="content" id="notices"><div className="sectionhead"><div><span>OPEN CALLS</span><h2>{reviewOnly?'검토 후보 · 공모 여부 미확인':showSaved?'관심 목록 · 본문 확인 공모사업':'본문 확인 공모사업'}</h2></div><p>{filtered.length}건 표시 · {mode==='live-db'?'실제 수집 자료':mode==='connecting'?'불러오는 중':'연결 확인 필요'}</p></div><p>공모사업 확인은 접수 중이라는 뜻이 아닙니다. 제목만 선별된 자료는 검토 후보에서 따로 확인하세요.</p><div className="filters" role="group" aria-label="검증 상태"><button aria-pressed={!reviewOnly} className={!reviewOnly?'selected':''} onClick={()=>setReviewOnly(false)}>본문 확인 {verifiedCount}</button><button aria-pressed={reviewOnly} className={reviewOnly?'selected':''} onClick={()=>setReviewOnly(true)}>검토 후보 {pendingCount}</button></div><div className="filters" role="group" aria-label="공고를 낸 기관 유형"><span>발행기관 유형</span>{groups.map(g=><button className={group===g?'selected':''} onClick={()=>setGroup(g)} key={g}>{g}</button>)}<label className="regionFilter"><span>지역</span><select value={region} onChange={e=>setRegion(e.target.value)} aria-label="지역 필터">{regions.map(r=><option key={r}>{r}</option>)}</select></label></div>
      ...
      {mode==='live-db'&&<DataQualityPanel items={baseFiltered.map(n=>n.details||{})}/>}
      <AdvancedSearch items={baseFiltered} value={advanced} onChange={setAdvanced}/>
      {filtered.length?<div className="noticegrid">{filtered.map((n,i)=><article key={n.id} className="notice"><div className="meta"><span className={`tag t${i%4}`}>{n.tag}</span><button onClick={()=>toggle(n.id)} aria-label="관심 공모 저장">{saved.includes(n.id)?'★':'☆'}</button></div>{n.relatedCount>0&&<div className="relatedBadge" title={n.relatedSources.join(' · ')}>관련 공고 {n.relatedCount}건 연결</div>}{<div className="reviewBadge">{verification(n).status==='verified'?'본문 확인 공모사업':'검토 후보 · 공모 여부 미확인'}</div>}{verification(n).status==='candidate'&&<div className="reviewActions"><button onClick={()=>review(n.id,'excluded')}>목록 제외</button></div>}{verification(n).evidence&&<details><summary>공모사업 확인 근거</summary><p>목적 · {verification(n).evidence?.purpose}</p><p>대상 · {verification(n).evidence?.audience}</p><p>지원 · {verification(n).evidence?.support}</p><p>신청 · {verification(n).evidence?.application}</p><small>본문 확인일 {verification(n).checkedAt?.slice(0,10)} · 이후 변경 가능, 접수기간 원문 재확인</small></details>}<p>{n.group} · {n.org}</p><h3><a href={n.url} target="_blank" rel="noreferrer">{n.title}</a></h3><div className="audience">지원대상 · {n.audience}</div><NoticeFacts details={n.details}/><a className="source" href={n.url} target="_blank" rel="noreferrer">{n.source} 원문 ↗</a></article>)}</div>:<div className="empty">{mode==='connecting'?'공고 목록을 불러오는 중입니다.':mode==='fallback'?'연결 복구 후 실제 공고를 표시합니다.':'현재 조건의 자료가 없습니다. 본문 확인 자료가 없으면 검토 후보 탭을 확인하세요.'}</div>}
    </section>
    <BojoBackfillPanel/>
