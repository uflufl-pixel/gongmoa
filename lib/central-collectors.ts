...
function identity(raw:string,c:Config){
  ...
  if(c.id==='khs-board'){
    ...
    if(path==='/multiBbz/selectMultiBbzView.do'&&u.searchParams.get('bbzId')==='newpublic'&&no&&/^\d+$/.test(no)){
      id=u.searchParams.get('id');u.pathname=path;u.search='';
      u.searchParams.set('bbzId','newpublic');u.searchParams.set('id',id||'');u.searchParams.set('no',no);u.searchParams.set('mn','NS_01_01');
    }
  }
...
export function parseCentralBoard(body:string,c:Config){
  ...
  if(c.format==='kasa'){
    ...
    for(const match of body.matchAll(/<item>[\s\S]*?<\/item>/g)){
      ...
      rows.push({title:c.id==='mnd-board'?field('title').replace(/\}$/, '').trim():c.id==='kma-board'?text(field('title')):field('title'),link:field('link'),posted:c.id==='mogef-board'?'':field('pubDate'),description:c.id==='saemangeum-board'?field('content:encoded'):['mods-board','mpm-board','mogef-board'].includes(c.id)?field('description'):undefined});
    }
    ...
    for(const match of body.matchAll(/<tr\b[^>]*>[\s\S]*?<\/tr>/g)){
      const cell=match[0].match(/<td data-column="제목">([\s\S]*?)<\/td>/);if(!cell)continue;
      const a=cell[1].match(/<a href="([^"]+)" class="b_tit" title="([^"]+)">/);
      if(!a)throw new Error('국가유산청 제목·링크 확인 필요');
      rows.push({title:text(a[2]),link:text(a[1]),posted:text(match[0].match(/data-column="등록일">([\s\S]*?)<\/td>/)?.[1]||'')});
    }
