...
const attachmentUrl='https://www.smtech.go.kr/front/comn/AtchFileDownload.do';
const attachmentId='EC1A1CEE2A787A200CE45338FD3959C5';
const attachmentHash='67477300268092402796cd030ef18f7374e1b0c7bbfeccc60b08a875f5db8706';
...
export async function fetchSmtechBundle(fetcher:typeof fetch=fetch){
  const headers={accept:'text/html','user-agent':'GongmoaSourceMonitor/1.1 (+https://gongmoa.uflufl.chatgpt.site)'};
  const list=await fetcher(smtechSource.url,{headers,redirect:'manual',signal:AbortSignal.timeout(20000)});if(!list.ok)throw new Error(`SMTECH HTTP ${list.status}`);const listText=await list.text();if(listText.length<50_000||listText.length>300_000)invalid();
  const detail=await fetcher(detailUrl,{headers,redirect:'manual',signal:AbortSignal.timeout(20000)});if(!detail.ok)throw new Error(`SMTECH HTTP ${detail.status}`);const detailText=await detail.text();if(detailText.length<50_000||detailText.length>350_000)invalid();
  const attachment=await fetcher(attachmentUrl,{method:'POST',headers:{...headers,accept:'*/*','content-type':'application/x-www-form-urlencoded',origin:'https://www.smtech.go.kr',referer:detailUrl},body:new URLSearchParams({atchFileId:attachmentId}),redirect:'manual',signal:AbortSignal.timeout(20000)});
  if(!attachment.ok)throw new Error(`SMTECH 첨부 HTTP ${attachment.status}`);const bytes=new Uint8Array(await attachment.arrayBuffer());if(bytes.length<10_000||bytes.length>10_000_000)invalid();
  const digest=await crypto.subtle.digest('SHA-256',bytes),hash=[...new Uint8Array(digest)].map(x=>x.toString(16).padStart(2,'0')).join('');
  return JSON.stringify({list:listText,detail:detailText,attachment:{size:bytes.length,magic:[...bytes.slice(0,8)],hash}});
}
export function collectSmtechBundle(input:string,knownIds:string[]=[],now=new Date()){
  let bundle:{list?:unknown;detail?:unknown;attachment?:unknown};try{bundle=JSON.parse(input);}catch{invalid();}if(typeof bundle.list!=='string'||typeof bundle.detail!=='string')invalid();
  ...
  const attachment=bundle.attachment as {size?:unknown;magic?:unknown;hash?:unknown}|undefined;
  if(!attachment||attachment.size!==85_504||JSON.stringify(attachment.magic)!=='[208,207,17,224,161,177,26,225]'||attachment.hash!==attachmentHash)invalid();
