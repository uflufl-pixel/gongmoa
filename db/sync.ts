...
export async function syncOfficialSources(requestedSourceIds?:readonly string[]) {
  ...
  const fipa=inspected.find(x=>x.check.sourceId==='fipa-education');
  if(fipa?.body&&fipa.check.outcome==='success'){
    try{const known=await db.select({id:notices.externalId}).from(notices).where(eq(notices.sourceId,'fipa-education'));const parsed=collectFipaBundle(fipa.body,known.map(x=>x.id));centralItems.push(...parsed.items);fipa.check.message=`전용 목록 ${parsed.parsedRows}행 · 상세 ${parsed.parsedDetails}건 · 현재·추적 교육공모 ${parsed.items.length}건`;}
    catch{fipa.check.outcome='parser_error';fipa.check.message='한국어촌어항공단 기술교육 구조 확인 필요';}
  }
  const kofpi=inspected.find(x=>x.check.sourceId==='kofpi-support');
  ...
  const koica=inspected.find(x=>x.check.sourceId==='koica-youth-contest');
  ...
  const koreahana=inspected.find(x=>x.check.sourceId==='koreahana-opportunities');
  ...
  const koipaPatent=inspected.find(x=>x.check.sourceId==='koipa-patent');
  if(koipaPatent?.body&&koipaPatent.check.outcome==='success'){
    try{const known=await db.select({id:notices.externalId}).from(notices).where(eq(notices.sourceId,'koipa-patent'));const parsed=collectKoipaPatentBundle(koipaPatent.body,known.map(x=>x.id));centralItems.push(...parsed.items);koipaPatent.check.message=`공식 일반·지역 상세/PDF ${parsed.parsedPages}건 감사 · 현재·추적 특허분쟁 지원 ${parsed.items.length}건`;}
    catch{koipaPatent.check.outcome='parser_error';koipaPatent.check.message='한국지식재산보호원 특허분쟁 공고 구조 확인 필요';}
  }
  const koipaBrand=inspected.find(x=>x.check.sourceId==='koipa-brand');
  ...
  const kista=inspected.find(x=>x.check.sourceId==='kista-opportunities');
  if(kista?.body&&kista.check.outcome==='success'){
    try{const known=await db.select({id:notices.externalId}).from(notices).where(eq(notices.sourceId,'kista-opportunities'));const parsed=collectKistaBundle(kista.body,known.map(x=>x.id));centralItems.push(...parsed.items);kista.check.message=`공식 지원기관 목록·상세·PDF ${parsed.parsedPages}건 감사 · 현재·추적 IP솔루션 지원 ${parsed.items.length}건`;}
    catch{kista.check.outcome='parser_error';kista.check.message='한국특허전략개발원 지원기관 사업공고 구조 확인 필요';}
  }
